...
func (m *mqttClient) handleIncomingMqtt(mqtt_client mqtt.Client, msg mqtt.Message) {
	...
	if strings.HasPrefix(msg.Topic(), m.topic("/set/register/")) {
		...
		durMap := map[string]byte{"10": 0x0, "20": 0x10, "30": 0x20, "on": 0x20, "off": 0x0}
...
var wrongBatteryLevelCount int = 0
...
func (m *mqttClient) publishRegister(msg *protocol.PhevMessage) {
	...
	case *protocol.RegisterBatteryLevel:
		if reg.Level <= 100 {
			if (reg.Level == 0 || reg.Level == 5) {
				wrongBatteryLevelCount++
			} else {
				wrongBatteryLevelCount = 0
			}

			if wrongBatteryLevelCount == 0 || wrongBatteryLevelCount > 30 {
